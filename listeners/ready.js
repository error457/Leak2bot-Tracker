...
module.exports = class ReadyListener extends Listener {
    ...
    async exec() {
        ...
        console.log(yellow(textSync('Leak2bot tracker', { horizontalLayout: 'full' })));
        console.log(green('Bot allumé!\n---\n'
        ...
        if(botstatus.enabled === true) {
            if(botstatus.activity_type.toUpperCase() == 'STREAMING') {
                client.user.setPresence({activity: {name: botstatus.activity_text, type: botstatus.activity_type.toUpperCase(), url: botstatus.activity_url}, status: botstatus.status.toLowerCase() || 'dnd'});
            } else {
                client.user.setPresence({activity: {name: botstatus.activity_text, type: botstatus.activity_type.toUpperCase()}, status: botstatus.status.toLowerCase() || 'dnd'});
            };
        };
