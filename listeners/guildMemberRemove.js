...
module.exports = class GuildMemberAddListener extends Listener {
    ...
    async exec(member) {
        ...
        if(!user || !user.inviter) return welcomeChannel.send(`${member.user.tag} a quitté le serveur mais je ne sais pas qui l'a invités.`);
        if(user.inviter === 'VANITY') return welcomeChannel.send(`${member.user.tag} quitté le serveur, il avais rejoint depuis le lien custom.`);
        ...
        if(!inviter) {
            ...
            return welcomeChannel.send(`${member.user.tag} a quitté le serveur, il a été inviter par **${inviter.tag}**`);
        };
