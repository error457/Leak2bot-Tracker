...
module.exports = class GuildMemberAddListener extends Listener {
    ...
    async exec(member) {
        ...
        if(member.user.bot) return welcomeChannel.send(`${member.toString()} rejoint le serveur en utilisant le flux OAuth.`).catch(err => console.log(err));
        ...
        if(!usedInvite) return welcomeChannel.send(`${member.toString()} a rejoint le serveur mais je ne sais pas comment. Peut-être une invitation temporaire?`).catch(err => console.log(err));
        if(usedInvite.code === member.guild.vanityURLCode) {
            welcomeChannel.send(`${member.toString()} a rejoint le serveur en utilisant le lien de vanité!`);
